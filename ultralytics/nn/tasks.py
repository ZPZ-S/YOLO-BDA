...
DETECT_CLASS = (Detect_DyAlignHead)
V10_DETECT_CLASS = (v10Detect,)
SEGMENT_CLASS = (Segment)
POSE_CLASS = (Pose, Pose_DyAlignHead)
OBB_CLASS = (OBB, OBB_DyAlignHead)
C3K2_CLASS = (C3k2_EFTM)
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in ((
            ...
            C1, C2, C2f, C3k2, ELAN1,  AConv, SPPELAN, C2fAttn, C3, C3TR, C3Ghost, DWConvTranspose2d,
            C3x, RepC3, PSA, SCDown, C2fCIB, KWConv, 
            SimpleStem, VisionClueMerge, VSSBlock_YOLO, XSSBlock,
            RFAConv, RFCBAMConv, RFCAConv
        ) + C3K2_CLASS):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            args = [ch[f], *args]
            c2 = args[0]
        elif m in (HGStem, HGBlock, EIEStem):
            ...
            c2 = m.channel
                   ...
                   SEAttention, CPCA,  EfficientAttention, MPCA, deformable_LKA,
                   EffectiveSEModule, LSKA, SegNext_Attention, DAttention, MLCA, TransNeXt_AggregatedAttention,
                   FocusedLinearAttention, LocalWindowAttention,  
                    CAA, ELA,  AFGCAttention, }:
            c2 = ch[f]
            args = [c2, *args]
            # print(args)
        ...
        if isinstance(c2, list) and m not in {ChannelTransformer, CrossLayerChannelAttention, CrossLayerSpatialAttention}:
            is_backbone = True
            m_ = m
            m_.backbone = True
        else:
